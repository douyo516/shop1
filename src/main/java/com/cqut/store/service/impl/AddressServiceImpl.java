...
import com.cqut.store.mapper.AddressMapper;
...
public class AddressServiceImpl implements IAddressService {
    ...
    @Override
    public void updateAdd(Integer uid, Address address, String username) {
        QueryWrapper<Address> qw = new QueryWrapper<>();
        qw.eq("aid",address.getAid());
        Address address1 = addressMapper.selectOne(qw);
        UpdateWrapper<Address> upqw = new UpdateWrapper<>();
        upqw.eq("aid",address.getAid());
        Date date = new Date();
        String provinceName = districtService.getNameByCode(address.getProvinceCode());
        String cityName = districtService.getNameByCode(address.getCityCode());
        String areaName = districtService.getNameByCode(address.getAreaCode());
        ...
        address.setModifiedUser(username);
        int update = addressMapper.update(address,upqw);

    }

    @Override
    public Address getAddSubmit(Integer aid) {
        QueryWrapper<Address> qw = new QueryWrapper<>();
        qw.eq("aid",aid);
