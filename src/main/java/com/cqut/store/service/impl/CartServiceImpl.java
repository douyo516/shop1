...
import com.cqut.store.service.ex.AccessDeniedException;
import com.cqut.store.service.ex.CartNotFoundException;
...
public class CartServiceImpl implements ICartService {
    ...
    @Override
    public List<Cart> selectMyCartById(Integer uid) {
    ...
    public Integer reduceNum(Integer cid, Integer uid, String username) {
        ...
        Date now = new Date();
        Integer row2 = cartMapper.updateNumByCid(cid, num, username, now);
        if (row2!=1){
            throw new InsertException("修改商品数量时出现未知错误，请联系系统管理员");
        }
        // 调用findByCid(cid)根据参数cid查询购物车数据
...
//        }
    ...
    @Override
    public Integer updateNum(Integer cid, Integer uid, String username, Integer amount) {
        QueryWrapper<Cart> qw = new QueryWrapper<>();
        ...
        Integer num=amount;
        Date now = new Date();
        Integer row2 = cartMapper.updateNumByCid(cid, amount, username, now);
        if (row2!=1){
            throw new InsertException("修改商品数量时出现未知错误，请联系系统管理员");
        }
        return amount;
    ...
    public void deleteCart(Integer cid, Integer uid) {
        ...
        int delete = cartMapper.delete(qw);
    }
}
